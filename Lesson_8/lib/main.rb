...
# Main Menu to operate trains, routes and stations
class Main
  include MenuHelper
  ...
  def train_menu
    loop do
      ...
      when 2 then movement_menu
      when 3 then wagons_menu
      when 4 then train_wagon_info
      when 0 then break
      else error
      end
    end
  ...
  def train_wagon_info
    if @trains.empty?
      ...
      train.wagon do |wagon|
        puts "Wagon Type: #{wagon.type}, Availability: #{wagon.available_capacity}, Occupied: #{wagon.capacity_usage}"
      end
    end
  end
  ...
  def station_control
    print "\n>>> STATION CONTROL <<<\n"
    puts "Enter Station number for editing"
    show_list(@routes)
