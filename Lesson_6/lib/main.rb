...
class Main
  ...
  def create_train
    puts "\n>>> TRAIN CREATION <<<"
    puts 'Number of your new train? '
    train_num = gets.to_s
    puts "\nSpecify the type of your train? "
    show_menu(TRAIN_TYPE_MENU)
    case gets.to_i
    when 1 then @trains << PassengerTrain.new(train_num)
    when 2 then @trains << CargoTrain.new(train_num)
    end
    puts "\nYou've successfully created a Train #{train_num}!"
  ...
  def add_wagon_option(train)
    ...
    puts "You've successfully added a wagon"
  end
  ...
  def remove_wagon_option(train)
    if train.wagons.empty?
      ...
      puts 'Choose the wagon to remove from: '
